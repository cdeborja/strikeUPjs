...
/***/ function(module, exports, __webpack_require__) {
	  ...
	  var DOMstrikecollection = __webpack_require__(3);
	  ...
	  var check = function () {
	    var $suis = $su('li');
	    $suis.each ( function ($sui) {
	      if (stateList.includes($sui.innerHTML)) {
	        $sui.classList.add("green");
	      } else {
	        $sui.classList.add("red");
	      }
	    });
	  };
	
	  var submit = function (e) {
	    if ($su(".submit-field").strikeUps[0].value !== "") {
	  ...
	  $su(function(){
	    ...
	    $su('button.clear-answers').on('click', clearAnswers);
	    $su('button.add').on('click', submit);
	    $su('button.add').on('keypress', submit);
